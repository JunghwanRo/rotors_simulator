...
This simulator version has been adapted from the [NTNU ARL's fork](https://github.com/ntnu-arl/rotors_simulator), which in turn is derived from the original [RotorS](https://github.com/ethz-asl/rotors_simulator) simulator developed by the Autonomous Systems Lab (ASL) at ETH Zurich.
...
```
 ...
 3. Build your workspace with `catkin_make_isolated` (therefore you need `catkin_make_isolated`)
...
With ROS Melodic, Gazebo `v9.x` is required (which is installed by default with ROS Melodic). However, it is **recommended to install at least Gazebo `v9.1`** to avoid a critical error.